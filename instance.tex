...
Let $P$ in \Cref{assmp:lsa}-~\ref{dist} be a positive definite distribution over $\C^{\ncn}$. Then
\begin{align}
\EE{\norm{\thh_t-\theta^*}^2}
\leq
...
Let $P$ in \Cref{assmp:lsa}-~\ref{dist} be AS. Define $\gamma_t\eqdef U^{-1}\theta_t,\,,\gamma^*\eqdef U^{-1}\theta^*$, and suppose if $P_U$ is positive definite, then
\begin{align}
\EE{\norm{\gh_t-\gamma^*}^2}
\leq
...
\paragraph{Error Recursion and Contraction Maps:}
...
\end{align}
Notice that \eqref{eq:spectralrand} places our definitions of $\rhod{P}$ and $\rhos{P}$ in \Cref{def:contract}. Here the subscripts $s$ and $d$ stand for \emph{stochastic} and \emph{deterministic}, since they are associated with the spectral radius of stochastic and deterministic matrices respectively.\\
...
\end{align*}
and thanks to \Cref{lm:pd} we know that when $A_P$ is positive definite it is possible to choose an $\alpha$ such that $\EE{\norm{I-\alpha A_t}}<1$.
\paragraph{Similarity Transformation:} When $A_{P^{AS}}$ is known only to be AS, it is possible that there exists no choice of $\alpha>0$ for which $\rhos{P^{AS}}>0$ and $\rhod{P^{AS}}>0$ (see \Cref{ex:matrices}). However, it is potentially possible to re-write the recursion in a different basis via a similarity transformation $\gamma_t=U^{-1 }\theta_t$, and $z_t\eqdef U^{-1} e_t$
...
\end{align}
where $J_t\sim P_U$ and thanks to \Cref{lm:simtran} we know that $P_U$ is PD and from \Cref{lm:pd} we know that there exists $\alpha$ such that $\EE{\norm{I-\alpha J_t}}<1$. We note here that a similarity transformation does not result in \eqref{eq:errrecsimtran} being a diagonal system. However, \Cref{lm:simtran} side steps non-diagonizablility by taking refuge in Jordon decomposition theorem in linear algebra (see Proof of \Cref{lm:simtran} in the Appendix).
\subsection{Result Discussion}
We first discuss implications of \Cref{thm:pdrate}.
