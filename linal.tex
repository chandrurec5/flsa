...
%\item Define constants $\EE{\norm{\zeta_t}^2}\eqdef\sigma_1^2=\sigma_A^2\norm{\ts}^2+\sigma_b^2$ and $\EE{\norm{M_t\zeta_t}}\eqdef \sigma_2^2=\sigma_A^2\norm{\ts}$.
...
Let $P$ be a distribution over $\R^d\times \R^{\dcd}$ as in \Cref{assmp:lsa} and let $U$ be chosen according to \Cref{lm:hur}. Define $\gamma_t\eqdef U^{-1}\theta_t,\,,\gamma^*\eqdef U^{-1}\theta^*$, then
\begin{align}
\EE{\norm{\gamma_t-\gamma^*}^2}
\leq
...
\end{align*}
where $z_t=\gamma_t-\gamma^*$, $\Lambda_t=U^{-1}A_t U$ and $H_t=U^{-1}\zeta_t$. Note that the error recursion in $z_t$ might involve complex computations (depending on whether $U$ has complex entries or not), and hence \eqref{eq:lsacmplx} and \Cref{assmp:lsacmplx} are useful in analyzing $z_t$.
We know that $\EE{\norm{H_t}^2}\leq \norm{U^{-1}}^2\EE{\norm{\zeta_t}}$ and $\EE{\norm{\Lambda_t H_t}}=\EE{\norm{U^{-1}A_t UU^{-1}\zeta_t}}=\EE{\norm{U^{-1}A_t \zeta_t}}\leq \norm{U^{-1}}\EE{\norm{A_t\zeta_t}}=\norm{U^{-1}}\sigma_2^2$. We follow through the steps of \Cref{th:pdrate} to obtain
\begin{align}
\E[\norm{\zh_t}^2]
