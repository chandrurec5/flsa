...
{\em (ii)} the noise is ``structured'', i.e., the variance \todoc{What exactly is this?} 
...
Thus, \Cref{th:rate} applies to all of TD(0)/TDC/GTD/GTD2/GTDMP with CS-PR in all cases considered in the literature.
Finite time performance of TDC was unkown in prior literature and our work provides a first step by understanding TDC with CS-PR. \todoc{I think this could be considered a major result if the Hurwitz condition can be shown to hold.}
...
However, it is important to note that \citet{korda-prashanth} handle the Markov noise case which is not dealt with in this paper. \todoc{How about others? We should mention if others also assume iid.}
\begin{comment}
