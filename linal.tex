...
\begin{comment}
\begin{lemma}
Given a matrix $A\in \C^{\ncn} there exists a \emph{characteristic} polynomial denoted by $\chi_A=(\lambda_1 \I-A)^{n_1}\ldots(\lambda_{k} \I-A)^{n_k}$, where $\lambda_i,\,i=1,\ldots,k (k\leq n)$ are the $k$ distinct eigenvalues of the matrix $A$ such that $\sum_{i=1}^k n_i=n$.
\end{lemma}
\begin{lemma}
...
Let $A\in \C^{\ncn}$ and $\{\lambda_i\in \C,i=1,\ldots,k\leq n \}$ denote its $k$ distinct eigenvalues.
There exists a complex matrix $V\in \C^{\ncn}$ such that $A=V\tL V^{-1}$, where
...
&>\frac{\re{\lambda_i}}{2}\left(\sum_{i=1}^n \md{x(i)+x(i+1)}^2 \right)\\
&> 0
\end{align*}
\end{proof}
...
Let $P$ in \Cref{lsadist} be a positive definite distribution over $\C_B^{\ncn}$. Then
\begin{align}
\EE{\norm{\thh_t-\theta^*}^2}
\leq
...
Let $t>i\ge 1$, $x,y\in \C^n$ be $\F_{i}$-measurable random vectors. Then,
\begin{align*}
